...
public class TaskFiveDirectTest {
    static final Logger logger = LoggerFactory.getLogger(TaskFiveDirectTest.class);
    ...
    @Test
    void testBalanceEndpoint() throws Exception {
        logger.info("=========================================");
        logger.info("TASK 5 DIRECT TEST - TESTING BALANCE ENDPOINT");
        logger.info("=========================================");
        ...
        // Test valid user
        Long testUserId = 5L; // waldorf's ID
        String url = baseUrl + "/balance?userId=" + testUserId;
        Balance balance = testRestTemplate.getForObject(url, Balance.class);
        
        if (balance != null) {
            logger.info("Balance for user {}: {}", testUserId, balance.getBalance());
        }
        
        // Test invalid user
        Long invalidUserId = 999L;
        String invalidUrl = baseUrl + "/balance?userId=" + invalidUserId;
        Balance invalidBalance = testRestTemplate.getForObject(invalidUrl, Balance.class);
        
        if (invalidBalance != null) {
            logger.info("Balance for invalid user {}: {} (should be 0)", invalidUserId, invalidBalance.getBalance());
        }
        ...
        // Look for the BEGIN/END snippet that the task expects
